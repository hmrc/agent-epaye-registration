...
class AgentEpayeRegistrationRepositoryISpec extends BaseRepositoryISpec with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  "AgentEpayeRegistrationRepository" should {
    "create a AgentReference record" in {
      ...
      await(repository.collection.find(Filters.equal("agentReference", "HX2000")).head()) mustBe AgentReference(
        "HX2000"
      )
    }
    ...
    "create new records and generate a new unique Agent PAYE Reference code" in {
      ...
      await(repository.create(regRequest))
      await(repository.collection.find(Filters.equal("agentReference", "HX2001")).head()) mustBe AgentReference(
        "HX2001"
      )

      await(repository.create(regRequest))
      await(repository.collection.find(Filters.equal("agentReference", "HX2002")).head()) mustBe AgentReference(
        "HX2002"
      )
