...
class AgentEpayeRegistrationValidatorSpec extends UnitSpec {
  ...
  "validate should pass" when {
    ...
    "the email address is unusual but valid" in {
      validate(regRequest.copy(emailAddress = Some("a+b@c.com"))) shouldBe Valid(())
      validate(regRequest.copy(emailAddress = Some("a_b@c.com"))) shouldBe Valid(())
      validate(regRequest.copy(emailAddress = Some("a.b@c.com"))) shouldBe Valid(())
    }
  }
  ...
  "validate should not pass" when {
    ...
    "the email address is longer than 129 characters" in {
      validate(regRequest.copy(emailAddress = Some(s"${padField(132)}@example.org"))) shouldBe
